...
describe('WorldSnapshotPacket', () => {
  ...
  it('round-trips with a single entity, including entityType', () => {
    const packet: WorldSnapshotPacket = {
      serverTick: 100,
      entities: [{ entityId: 7, entityType: 1, x: 12.5, y: -3.25, speed: 200 }],
    };
  ...
  it('round-trips with many entities preserving order and per-entity entityType', () => {
  ...
  it('produces a header with the correct opcode and payload length', () => {
    const buffer = encodeWorldSnapshot({
      serverTick: 0,
      entities: [{ entityId: 1, entityType: 0, x: 0, y: 0, speed: 0 }],
    });
    beginRead(buffer);
    const header = readHeader();
    endRead();
    expect(header.opcode).toBe(Opcode.WorldSnapshot);
    expect(header.length).toBe(6 + 17);
  });
});
