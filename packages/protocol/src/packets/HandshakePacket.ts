...
/**
 * Server -> Client, sent once immediately after a connection is accepted. Carries the
 * world's bounds so the client never hardcodes them — different regional server
 * instances (or, later, different maps) can have different sizes, and the client learns
 * its instance's actual bounds over the wire instead of assuming a shared constant.
 */
...
export function decodeHandshake(): HandshakePacket {
  ...
  return { assignedEntityId, tickRate, worldMinX, worldMaxX, worldMinY, worldMaxY };
}
